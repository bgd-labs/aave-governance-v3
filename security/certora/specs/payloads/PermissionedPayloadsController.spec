...
/* ============================================================================================================
   This file is a duplication of PayloadsController.spec, with the proper adaptations made in order to fit it for
   the PermissionedPayloadsController contract.
   To run the rule of this file: certoraRun security/certora/confs/payloads/verifyPermissionedPayloadsController.conf
   
   Above the rules of PayloadsController.spec, we added (at ht eend of this file) several rules for the following 
   properties (see docs/properties.md):
...
methods {
  ...
  function payloadsManager() external returns (address) envfree;
}
...
rule consecutiveIDs(method f) filtered {f->
    !f.isView
...
rule action_immutable_fixed_size_fields(method f) filtered {f ->
...
rule executor_of_maximumAccessLevelRequired_exists(method f) filtered {f->
    !f.isView
    && f.selector != sig:updateExecutors(IPayloadsControllerCore.UpdateExecutorInput[]).selector
    }
{
...
rule no_late_cancel(method f) filtered {f ->
    f.selector != sig:updateExecutors(IPayloadsControllerCore.UpdateExecutorInput[]).selector
} {
  env e1; env e2; env e3;
  calldataarg args;
...
rule no_transition_beyond_state_gt_3(method f) filtered {f ->
    !f.isView
    && f.selector != sig:updateExecutors(IPayloadsControllerCore.UpdateExecutorInput[]).selector
} {
...
rule no_transition_beyond_state_variable_gt_3(method f) filtered {f ->
    f.selector != sig:updateExecutors(IPayloadsControllerCore.UpdateExecutorInput[]).selector
} {
  env e;
  calldataarg args;
...
rule payload_state_transition_post_state(method f) filtered {f ->
    !f.isView
  ...
  assert (e1.block.timestamp == e3.block.timestamp && state1 != state2) =>
    ((state1 == IPayloadsControllerCore.PayloadState.None => state2 == IPayloadsControllerCore.PayloadState.Queued)
    );
  ...
  assert (e1.block.timestamp < e3.block.timestamp && state1 != state2) =>
    (state1 == IPayloadsControllerCore.PayloadState.None =>
      (state2 == IPayloadsControllerCore.PayloadState.Queued
      || state2 == IPayloadsControllerCore.PayloadState.Expired
      )
    );
  
  assert (e1.block.timestamp < e3.block.timestamp && state1 != state2) =>
    (state1 == IPayloadsControllerCore.PayloadState.Queued =>
     (state2 == IPayloadsControllerCore.PayloadState.Executed
      || state2 == IPayloadsControllerCore.PayloadState.Cancelled
      || state2 == IPayloadsControllerCore.PayloadState.Expired
     )
    );
}
...
rule payload_state_transition_pre_state(method f) filtered {f ->
  ...
  requireInvariant null_state_variable_if_out_of_bound_payload(payloadId);
  requireInvariant expirationTime_equal_to_createAt_plus_EXPIRATION_DELAY(payloadId);
  requireInvariant state_is_never_CREATED(payloadId);
  ...
  assert (state1 != state2) =>
    (state2 == IPayloadsControllerCore.PayloadState.Queued => state1 == IPayloadsControllerCore.PayloadState.None);
  
  assert (state1 != state2) =>
    (state2 == IPayloadsControllerCore.PayloadState.Executed => state1 == IPayloadsControllerCore.PayloadState.Queued);
  
  assert (state1 != state2) =>
    (state2 == IPayloadsControllerCore.PayloadState.Cancelled => state1 == IPayloadsControllerCore.PayloadState.Queued);
  
  assert (e2.block.timestamp == e3.block.timestamp  && state1 != state2) =>
    (state2 == IPayloadsControllerCore.PayloadState.Expired => state1 == IPayloadsControllerCore.PayloadState.Queued);
}
...
/* ============================================================================================================
   The following rules and invariants cover the the folowing properties:
   - Supports payloads only with Level_1 access level.
   - Once executor config is created, it can't be changed except the delay configuration of the Level_1 executor.
   - Only payloads manager is permitted to create a Payload.
   - Payload creation is combined with queuing: once created, a Payload is automatically queued.
...
invariant state_is_never_CREATED(uint40 id)
  getPayloadStateVariable(id) != IPayloadsControllerCore.PayloadState.Created
  filtered {f ->
    f.selector != sig:initialize(address,address,IPayloadsControllerCore.UpdateExecutorInput[]).selector
    }

invariant payload_maximumAccessLevelRequired_is_LEVEL1(uint40 id, env e) 
  getMaximumAccessLevelRequired(id) == PayloadsControllerUtils.AccessControl.Level_null ||
  getMaximumAccessLevelRequired(id) == PayloadsControllerUtils.AccessControl.Level_1
  filtered {f ->
  f.selector != sig:initialize(address,address,IPayloadsControllerCore.UpdateExecutorInput[]).selector
}


rule executorConfig_cant_change_except_delay1(method f) filtered {f->
    !f.isView && !is_reverting_func(f)
    } {
  address executor1_before = getExecutorSettingsByAccessControl(PayloadsControllerUtils.AccessControl.Level_1).executor;
  address executor2_before = getExecutorSettingsByAccessControl(PayloadsControllerUtils.AccessControl.Level_2).executor;
  uint40 delay2_before = getExecutorSettingsByAccessControl(PayloadsControllerUtils.AccessControl.Level_2).delay;

  calldataarg args;
  env e;
  f(e,args);

  address executor1_after = getExecutorSettingsByAccessControl(PayloadsControllerUtils.AccessControl.Level_1).executor;
  address executor2_after = getExecutorSettingsByAccessControl(PayloadsControllerUtils.AccessControl.Level_2).executor;
  uint40 delay2_after = getExecutorSettingsByAccessControl(PayloadsControllerUtils.AccessControl.Level_2).delay;

  assert executor1_after == executor1_before;
  assert executor2_after == executor2_before;
  assert delay2_after == delay2_before;
}
...
rule only_payloadsManager_can_call_createPayload() {
  calldataarg args;  env e;
  createPayload(e,args);

  assert e.msg.sender== payloadsManager() ;
}


rule only_payloadsManager_or_guardian_can_call_cancelPayload() {
  calldataarg args;  env e;
  cancelPayload(e,args);

  assert e.msg.sender==payloadsManager() || e.msg.sender==guardian();
}


rule payloadsManager_can_call_cancelPayload() {
  calldataarg args;  env e;
  require e.msg.sender==payloadsManager();
  cancelPayload@withrevert(e,args);
  satisfy true;
}


rule guardian_can_call_cancelPayload() {
  calldataarg args;  env e;
